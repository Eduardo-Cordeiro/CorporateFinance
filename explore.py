...
run_query("SELECT DATA_BASE, CNPJ, SUM(CASE WHEN CONTA IN (11000000, 12000000) THEN saldo ELSE 0 END)/NULLIF(SUM(CASE WHEN CONTA = 41100000 THEN saldo ELSE 0 END), 0) AS teste FROM bp GROUP BY DATA_BASE, CNPJ")

#Média do índice de liquidez imediata de todas as cooperativas de crédito em cada mês de referência
run_query("""
    SELECT 
        DATA_BASE, 
        ROUND(AVG(indice), 4) AS media_indice
    FROM (
        SELECT 
            DATA_BASE, 
            CNPJ, 
            SUM(CASE WHEN CONTA IN (11000000, 12000000) THEN saldo ELSE 0 END)/
            NULLIF(SUM(CASE WHEN CONTA = 41100000 THEN saldo ELSE 0 END), 0) AS indice
        FROM bp
